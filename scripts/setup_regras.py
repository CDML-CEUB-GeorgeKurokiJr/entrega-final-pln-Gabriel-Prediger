...
def main():
    ...
    try:
        ...
        for cozinha in cozinhas:
            print(
                f"   - {cozinha['original_name']}: {cozinha['file_count']} arquivos → collection '{cozinha['collection_name']}'")
        ...
        try:
            ...
            for collection in collections:
                if collection['name'].startswith('cozinha_'):
                    print(
                        f"   - {collection['name']}: {collection.get('document_count', 0)} documentos, {collection.get('chunk_count', 0)} chunks")
        except Exception as e:
            print(f"⚠️  Erro ao listar collections: {e}")
